...
exports.login = async (req, res) => {
  ...
  try {
    const user = await User.findOne({ uname, role: { $ne: 'admin' } });
...
exports.adminlogin = async (req, res) => {
  const { uname, password } = req.body;
  try {
    const user = await User.findOne({ uname: uname, role: 'admin' });
    if (!user) return res.status(401).json({ message: 'Invalid credentials' });
...
exports.getPaginatedUsers = async (req, res) => {
  try {
    ...
    const limit = parseInt(req.query.limit) || 10; // Default to 5 if not provided
